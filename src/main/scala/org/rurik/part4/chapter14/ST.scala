package org.rurik.part4.chapter14
...
object STArray {
  ...
  def quicksortST(xs: List[Int]): List[Int] =
...
sealed abstract class STMap[S, A, B](implicit manifest: Manifest[A]) {
  protected def value: mutable.Map[A, B]

  def put(k: A, v: B): ST[S, Unit] = new ST[S, Unit] {
    def run(s: S): (Unit, S) = {
      value.put(k, v)
      ((), s)
    }
